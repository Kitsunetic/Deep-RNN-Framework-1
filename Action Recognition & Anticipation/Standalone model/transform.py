...
__all__ = ["Compose", "ToTensor", "ToPILImage", "Normalize", "Resize", "CenterCrop", "Pad",
           ...
           "ColorJitter", "RandomRotation", "RandomAffine", "Grayscale", "RandomGrayscale", "RandomCenterCrop"]
        ...
        if random.random() < self.p:
            if isinstance(img,  list):
                ...
                for i in range(len(img)):
                    re.append(center_crop(img[i], self.size))
                return re
...
class RandomGrayscale(object):
    ...
    def __call__(self, img):
        ...
        """
        if isinstance(img, list):
            num_output_channels = 1 if img[0].mode == 'L' else 3
        else:
        ...
        if random.random() < self.p:
            if isinstance(img, list):
                re = []
                for i in range(len(img)):
                    re.append(to_grayscale(img[i], num_output_channels=num_output_channels))
                return re
